...
- (void)loginWithSignInProvider:(AWSSignInProviderType)signInProviderType
    ...
    } else if (signInProviderType == AWSSignInProviderTypeTwitter) {
        self.currentSignInProvider = [AWSTwitterSignInProvider sharedInstance];
    }
...
- (void)resumeSessionWithCompletionHandler:(void (^)(id result, NSError *error))completionHandler {
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"Twitter"]) {
        self.currentSignInProvider = [AWSTwitterSignInProvider sharedInstance];
    }
